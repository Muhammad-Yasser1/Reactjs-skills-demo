	...
	useEffect(() => {
		setInterval(() => {
			...
			if (wasAppOnline.current && !isAppOnline.current) {
				dispatch(
					notify(
						"we are offline now but don't worry all your operations will be sync with database after you get back online",
						'error'
					)
				);
			} else if (!wasAppOnline.current && isAppOnline.current) {
				dispatch(
					notify('we are online now if you done any operation it will be synced with database now', 'success')
				);
			}
		}, everySeconds);
	}, []);

	return { wasAppOnline: wasAppOnline.current, isAppOnline: isAppOnline.current } as const;
};
